...
namespace mpc_car
{
    ...
    static constexpr bool use_phr_alm_for_nmpc_ = true;
    ...
    public:
                                   ...
                                   VectorX &grad_x)
        {
            ...
            // TODO: penalty constraints
            double grad_v = 0;
            const double mu1 = ALM_mu_(8 * k + 4); // -0.1 - v
            const double mu2 = ALM_mu_(8 * k + 5); // v - v_max_
            const double rho = ALM_rho_;
            cost += al_box_constrant(x(3), -0.1, v_max_, mu1, mu2, rho, grad_v);
            grad_x(3) += grad_v;
            return cost;
        }

        static inline double AL_objectiveFunc(void *ptrObj,
                                              const double *x,
                                              double *grad,
                                              const int n)
        {
        ...
        int solve_NMPC_PHR_AL(const VectorX &x0_observe)
        {
            historyInput_.pop_front();
            historyInput_.push_back(predictInput_.front());
            // x0_observe_ = x0_observe;
            ...
            // PHR-ALM
            {
                // choose ALM parameters
                ALM_mu_ = Eigen::VectorXd::Zero(8 * N_);
                ALM_rho_ = 1.0;
                ALM_gamma_ = 1.0;
                ALM_beta_ = 1e3;

                double cons_residual = 1e5, prec_residual = 1e5 const double cons_eps = -1e3, prec_eps = -1e3;
                ...
                auto Calculate_Optimal_Control_Sequence = [&]() -> void
                {
                    ...
                    double minObjective;
                    auto ret = lbfgs::lbfgs_optimize(m * N_, x, &minObjective, &AL_objectiveFunc, nullptr, nullptr, this, &lbfgs_params);
                };
                ...
                auto Update_Dual_Variables_And_Residual = [&]() -> void
                {
                    ...
                    for (int i = 0; i < N_; ++i)
                    {
                        ...
                        if (i < N - 1)
                        {
                            g(4) = -0.1 - xk_1(3);
                            g(5) = xk_1(3) - v_max_;
                            const double ddelta = inputs(i)(1) - inputs(i + 1)(1);
                            g(6) = -ddelta_max_ - ddelta;
                            g(7) = ddelta - ddelta_max_;
                        }

                        for (int j = 0; j < (i < N - 1) ? 8 : 4; ++j)
                        {
                            ALM_mu_(i + constrains_num * j) = std::max(0.0, ALM_mu_(i + constrains_num * j) + ALM_rho_ * g(j));
                    ...
                    double *grad_of_AL_x = (double *)vecalloc(m * N_ * sizeof(double));
                    double cost = AL_objectiveFunc(this, x, grad_of_AL_x, m * N_);
                ...
                // PHR-ALM main loop
                while (cons_residual >= cons_eps || prec_residual >= prec_eps)
                {
        ...
        {
            if (use_phr_alm_for_nmpc_) 
            {
                return solve_NMPC_PHR_AL(x0_observe);
            }
            historyInput_.pop_front();
            historyInput_.push_back(predictInput_.front());
            // x0_observe_ = x0_observe;
